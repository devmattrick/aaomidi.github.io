<!-- Load this via localhost:8100/performance or localhost:8100/p -->

<!DOCTYPE html>
<html>
<head></head>
<body>
<div id='warning'>If this screen is just blank, you should probably start the test server and load the tests at
    localhost:8101
</div>
...
<script src="/src/test/generated/performanceTests.umd.js"></script>
...
<script>
    ...
    [100, 1000, 2000, 5000].forEach(function (timeout) {
        setTimeout(function () {
            window.scrollTo(0, 0);
        }, timeout) // wait for it to start being dumb and scrolling weird to scroll back
    })
